<x-app-layout>
    <div class="container mx-auto p-4">
        <h1 class="text-2xl font-bold mb-6 text-white">Create User</h1>
        <form action="{{ route('user.store') }}" method="POST" class="space-y-4 bg-white p-6 rounded-lg shadow-md">
            @csrf
            <div>
            ...
            <button type="submit" class="w-full inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Create</button>
        </form>
    </div>
</x-app-layout>
